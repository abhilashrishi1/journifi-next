...
function filterByPeriod(trades, period) {
  ...
  if (period==='month') return trades.filter(t=>{const d=new Date(t.date);return d.getFullYear()===most.getFullYear()&&d.getMonth()===most.getMonth();});
  if (period==='week') {const c=new Date(most);c.setDate(c.getDate()-6);return trades.filter(t=>{const d=new Date(t.date);return d>=c&&d<=most;});}
  return trades;
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode==='login') {
        ...
        const {error} = await sb.auth.signUp({email,password,options:{data:{username:email.split('@')[0]}}});
        if (error) throw error;
    ...
    <div style={{minHeight:'100vh',background:T.bg,display:'flex',alignItems:'center',justifyContent:'center',padding:32}}>
      <div style={{width:'100%',maxWidth:360,display:'flex',flexDirection:'column',gap:20}}>
        <div style={{textAlign:'center',marginBottom:8}}>
          ...
          <div style={{fontSize:12,textTransform:'uppercase',letterSpacing:'0.12em',color:T.gold,fontFamily:"'IBM Plex Mono', monospace",marginTop:4}}>Personal Trade Journal</div>
        </div>
        ...
        <div style={{textAlign:'center',fontSize:11,color:T.inkMuted,fontFamily:"'IBM Plex Mono', monospace"}}>Your trades are private and encrypted.</div>
      </div>
    </div>
  );
}
...
function TradeRow({trade,note,onNoteChange,expanded,onToggle,maxLoss,C}) {
  const gain=trade.pnl>=0, violates=trade.pnl<-Math.abs(maxLoss);
  return (
    <div style={{borderBottom:`1px solid ${C.border}`}}>
      <button onClick={onToggle} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 4px',background:'transparent',border:'none',cursor:'pointer'}}>
  ...
  useEffect(()=>{
    const fetchAll=async()=>{
      ...
      await Promise.all(WATCHLIST.map(async sym=>{
        try{const res=await fetch(`https://finnhub.io/api/v1/quote?symbol=${sym}&token=${FINNHUB_KEY}`);results[sym]=await res.json();}catch(e){}
      }));
      setQuotes(results);setLoading(false);
    };
    fetchAll();
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:8}}>
      ...
      {loading?<div style={{textAlign:'center',padding:32,color:C.inkMuted}}>Loading...</div>:
        WATCHLIST.map(sym=>{
          const q=quotes[sym]||{},gain=(q.dp||0)>=0;
          return(
...
export default function JournifiApp() {
  ...
  useEffect(() => {
    ...
    const sb = getSupabase();
    const load = async () => {
      const {data,error} = await sb.from('trades').select('*, trade_notes(*)').eq('user_id',user.id).order('date',{ascending:false});
      if (!error&&data) {
        setTrades(data);
        const nm={};
        data.forEach(t=>{if(t.trade_notes&&t.trade_notes.length>0)nm[t.id]=t.trade_notes[0];});
        setNotes(nm);
      }
    };
    load();
  ...
  const handleNoteChange = async (tradeId, nextNote) => {
    ...
    const sb = getSupabase();
    await sb.from('trade_notes').upsert({trade_id:tradeId,user_id:user.id,setup:nextNote.setup||null,psychology:nextNote.psychology||null,notes:nextNote.text||null},{onConflict:'trade_id,user_id'});
  };

  const handleAddTrade = async (trade) => {
    const sb = getSupabase();
    const {data,error} = await sb.from('trades').insert({id:`manual-${Date.now()}`,user_id:user.id,date:trade.date,symbol:trade.symbol,exit_type:trade.exitType,pnl:trade.pnl}).select().single();
  ...
  return (
    <div style={{minHeight:'100vh',background:C.bg}}>
          ...
          <>
            ...
            <div style={{borderRadius:14,padding:14,display:'flex',gap:8,background:hexToRgba(C.gold,0.08),border:`1px solid ${hexToRgba(C.gold,0.3)}`}}>
              ...
              <div style={{fontSize:13,lineHeight:1.5,color:C.ink}}>
                {stats.total===0?'No trades yet.':stats.profitFactor<1&&stats.winRate>=45
                  ?`Win rate ${stats.winRate.toFixed(0)}% — solid. Leak is size: avg losses (${fmtPlain(stats.avgLoss)}) bigger than avg wins (${fmtPlain(stats.avgWin)}). Cut losers faster.`
                  :stats.profitFactor>=1
                  ?`Profit factor ${stats.profitFactor.toFixed(2)} — net positive. Tag trades to see which setup is carrying the period.`
      ...
      <div style={{position:'fixed',bottom:0,left:0,right:0,background:C.surface,borderTop:`1px solid ${C.border}`,display:'flex',zIndex:100,paddingBottom:'env(safe-area-inset-bottom)'}}>
        {[{id:'journal',icon:'📒',label:'Journal'},{id:'quotes',icon:'📈',label:'Quotes'},{id:'news',icon:'📰',label:'News'}].map(tab=>(
